...
Run "sudo rake gems:install".  That's it, you now have CQL support in your Blacklight.  
...
This plugin does *not* provide a full SRU server. However, a ZeeRex/SRU [[explain|http://explain.z3950.org/overview/index.html]] document is provided by the plugin to advertise, in machine-readable format, what CQL indexes (ie, search fields) are provided by the server, and what relations are supported on each search field. 
